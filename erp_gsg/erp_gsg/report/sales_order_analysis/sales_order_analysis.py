...
from frappe.utils import date_diff, flt, getdate, time_diff
...
def validate_filters(filters):
	...
	elif time_diff(to_time, from_time) < datetime.timedelta(0):
...
def get_conditions(filters):
	...
	if filters.get("from_time") and filters.get("to_time"):
		conditions += " and so.time between %(from_time)s and %(to_time)s"
...
def get_data(conditions, filters):
	...
	data = frappe.db.sql(
		...
		SELECT
			...
			so.name as sales_order, so.time,
